...
*#define vector real
      ...
      Subroutine cobrems(Emax,Epeak,ytilt,emitmr,
     +                   radt,dist,coldiam,polar,epol,mos)
      real Emax,Epeak,ytilt,emitmr,radt,dist,coldiam,epol,mos
      integer polar
      include 'cobrems.inc'
      integer i
      real c
...
c     a=5.43e-10               !dimension of silicon unit cell (m)
      ...
      mospread=mos             !crystal r.m.s. mosaic spread
      ...
      ecircpolar=epol          !electron beam circular polarization (signed)
...
c     mospread = mospread * 1e-6
      ...
      thy=ytilt        !rotation of crystal about y (second)
C-- require Epeak < Emax
      if (Epeak.ge.Emax) then
        return
      endif
C-- decide if you want total flux, linear polarized flux, 
C   or circular polarized flux
      polarflux=polar
...
2000  format(3f12.6)
      ...
      npoints=2
      do n=1,npoints
        phi=(n-0.5)*dpi/(2*npoints)
        S=S+REAL(dNcdxdp(x,phi))
      enddo
      dNcdx=2*dpi*S/npoints
      end
...
C  shape resembling the Bethe-Heitler result.
     ...
     +      * acceptance(theta2)
      if (polarflux.eq.2) then
        dNidxdt2 = dNidxdt2 * polarization(x,theta2,0.)
      endif
c     write(6,*) dNidxdt2
      end
...
c    +           real(Npara - Nperp) / real(Npara + Nperp)
      ...
      if (polarflux.eq.2) then
        theta2=0
        paverage=2*(1-x)/((1+theta2)**2*((1-x)**2+1) - 4*theta2*(1-x))
        polarization = sqrt(1-paverage**2) * ecircpolar
      endif
      end

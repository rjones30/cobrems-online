...
3. **vncserver**: you need to have the vncserver installed, so that vncserver is in the path. The ratetool.cgi script starts its own private Xvnc session on display 99 and pawX11 uses it to generate plots through the X11 graphical interface. If screen 99 is not available, change the display number in the code near the top of the ratetool.cgi script.
